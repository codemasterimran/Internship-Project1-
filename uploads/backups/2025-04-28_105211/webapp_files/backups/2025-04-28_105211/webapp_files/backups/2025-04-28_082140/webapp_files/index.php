...
if (isset($_GET['action']) && $_GET['action'] === 'update' && $screen === 'welcome') {
    ...
    $logContent .= "Previous Version: " . $currentWebAppVersion . "\n";
    ...
    // Update current version in session
    $_SESSION['current_version'] = $latestWebAppVersion;
    
    // Set success message
    $_SESSION['update_success'] = [
        'previous_version' => $currentWebAppVersion,
...
// Check if we have an updated version in session
$currentWebAppVersion = $_SESSION['current_version'] ?? $currentWebAppVersion;
...
function getLatestVersions() {
    ...
    // In production, this would come from your version API endpoint
...
$webAppUpdateAvailable = version_compare($currentWebAppVersion, $latestVersions['webapp_latest'], '<');
$webAppVersionStatus = '';
if ($webAppUpdateAvailable) {
    $webAppVersionStatus = 'outdated';
} elseif (version_compare($currentWebAppVersion, $latestVersions['webapp_latest'], '>')) {
...
<body>
    ...
    <div class="app-container">
        ...
        <?php if ($webAppUpdateAvailable && !isset($_SESSION['update_success'])): ?>
        ...
        <div class="screen sql-editor-screen active">
            ...
            <form method="post" class="sql-editor-form">
                <!-- SQL Content (hidden since we're auto-loading from file) -->
                <input type="hidden" name="sql_content" value="<?= htmlspecialchars($sqlContent) ?>">
                
                <div class="form-group">
                    <label>SQL File Loaded Automatically From:</label>
                    <div class="file-path-display">
                        <i class="fas fa-file-code"></i> 
                        D:/xamp/htdocs/uploads/test.sql
                    </div>
                </div>
